...
class tlTestPlanMetrics extends testplan
{
  ...
  function getExecStatusMatrixFlat($id, $filters=null, $opt=null) {
     ...
     *
     */
    function cus_get_MatrixForTestType($id, $db, $filters=null, $opt=null) {
        $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
        ...
        $rs = $this->db->get_recordset($sql);
        // new dBug($rs);
        // TODO: check lai cho nay neu co nhieu custome dada thi co sai khong?
        $testCaseClass = new testcase($db);
        foreach ($rs as &$testcase) {
            $testcfield = $testCaseClass->get_linked_cfields_at_design($testcase["tcase_id"], $testcase["tcversion_id"]);
            $testcase['testType'] = $testcfield["1"]["value"];
        }
        $ltx = null;
        if(!is_null($rs))
        {
